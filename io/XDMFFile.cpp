...
#include <mesh/Cell.h>
#include <mesh/DistributedMeshTools.h>
#include <mesh/Edge.h>
                                ...
                                bool append)
{
  ...
  if (boost::filesystem::exists(_filename) && append == true)
  {
  ...
  if (_xml_doc->select_node("/Xdmf/Domain").node().empty() || append == false)
  {
  ...
  if (truncate_hdf && (boost::filesystem::exists(get_hdf5_filename(_filename))))
  {
                     ...
                     const Encoding encoding)
{
  ...
  if (!mesh_node)
  {
    ...
    if (new_timegrid || parameters["rewrite_function_mesh"])
    {
                                           ...
                                           const Encoding encoding)
{
  ...
  else
  {
    ...
    if (boost::lexical_cast<std::int64_t>(dims_list[0]) != npoints
        || boost::lexical_cast<std::int64_t>(dims_list[1]) != (int)gdim)
    {
      dolfin_error("XDMFFile.cpp",
                   "add MeshValueCollection to file",
                   "Incompatible Mesh");
...
(MeshValueCollection<T>& mvc, std::string name)
{
  ...
  for (pugi::xml_node node: domain_node.children("Grid"))
  {
    ...
    if (value_node && (name == "" || name == value_node.attribute("Name").as_string()))
      {
        grid_node = node;
        break;
      }
                            ...
                            const Mesh& mesh)
{
  ...
  //               x_cell_dofs.end(),
  //               x_cell_dofs.begin(),
  //               std::bind(std::plus<std::size_t>(), offset));
...
std::vector<T> XDMFFile::compute_topology_data(const Mesh& mesh, int cell_dim)
{
  ...
  const int tdim = mesh.topology().dim();
  if (MPI::size(comm) == 1 || cell_dim == tdim)
  {
...
std::vector<T> XDMFFile::compute_quadratic_topology(const Mesh& mesh)
{
  ...
  if (geom.degree() != 2 || MPI::size(mesh.mpi_comm()) != 1)
  {
    dolfin_error("XDMFFile.cpp",
...
std::int64_t XDMFFile::get_num_cells(const pugi::xml_node& topology_node)
{
  ...
  if (tdims.size() != 2 && num_cells_topolgy == -1)
  {
    dolfin_error("XDMFFile.cpp",
  ...
  // and DataItem nodes
  if (num_cells_topolgy != -1 && tdims.size() == 2)
  {
                                  ...
                                  std::string name)
{
  ...
  [&] {
    for (pugi::xml_node node : domain_node.children("Grid"))
    {
      ...
      {
        if (attr_node
            && (name == "" || name == attr_node.attribute("Name").as_string()))
        {
          grid_node = node;
...
{
  if (encoding == Encoding::HDF5 && (!has_hdf5()))
  {
    dolfin_error("XDMFFile.cpp",
                 "write XDMF file",
  ...
  if (encoding == Encoding::ASCII && _mpi_comm.size() != 1)
  {
    dolfin_error("XDMFFile.cpp",
                 "write XDMF file",
