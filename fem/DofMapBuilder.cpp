...
#include <graph/BoostGraphOrdering.h>
                          ...
                          std::shared_ptr<const SubDomain> constrained_domain)
{
  ...
  const bool reorder = (distributed || reorder_ufc) ? true : false;
  ...
  const std::size_t bs = (global_dofs.empty() && reorder)
  ...
  const std::size_t global_dim)
{
  ...
  for (std::size_t i = 0; i < num_nodes_local; ++i)
  {
    if (shared_nodes[i] == -3 || shared_nodes[i] == -2)
    {
      // Send global index
      const std::size_t global_index = local_to_global[i];
      const std::size_t dest = MPI::index_owner(mpi_comm,
  ...
  const Mesh& mesh)
{
  ...
  for (FacetIterator f(mesh, "all"); !f.end(); ++f)
  {
    ...
    if ((!f->is_shared()) && (f->num_entities(D) == 2))
      continue;
  ...
  MPI_Comm mpi_comm)
{
  ...
  else if (ordering_library == "random")
  {
    ...
    std::shuffle(node_remap.begin(), node_remap.end(), engine);
  }
  else
  {
    dolfin_error("DofMapBuilder.cpp",
